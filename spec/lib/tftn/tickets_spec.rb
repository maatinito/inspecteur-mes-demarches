...
RSpec.describe Tftn::Tickets do
  ...
  let(:cours_field) { { 'id' => 1, 'name' => 'Nom du cours', 'type' => 'text' } }
  let(:date_field) { { 'id' => 2, 'name' => 'Date du cours', 'type' => 'date' } }
  let(:baserow_fields) { [cours_field, date_field] }
  ...
  before do
    ...
    allow(mock_table).to receive(:table_id).and_return(params[:id_table_baserow])
    allow(Baserow::Config).to receive(:table).with(params[:id_table_baserow], nil).and_return(mock_table)

    # Mock pour les appels à Baserow
    allow(mock_client).to receive(:list_fields).with(params[:id_table_baserow]).and_return(baserow_fields)
