...
describe 'Excel::FromRepetitions' do
  ...
  before do
    ...
    allow(controle).to receive(:instructeur_id_for).and_return(1) if respond_to?(:controle)
  end
  ...
  describe '#champ_cell_value' do
    ...
    it 'écrit une date de cellule sous forme de chaîne française' do
      allow(File).to receive(:exist?).with('modele.xlsx').and_return(true)
      controle = Excel::FromRepetitions.new(champ_cible: 'Export', modele: 'modele.xlsx')
