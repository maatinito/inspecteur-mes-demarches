...
RSpec.describe Diese::EtatPrevisionnel3Check do
  context 'DNs, sum copies are wrong' do
    ...
    let(:rate_message) do
      FactoryBot.build :message, field: field_name(field, 0), value: 'Mauvais Taux', message: 'message_taux_depasse40%'
    end
    let(:dn_messages) { LM.map { |msg| new_message(field_name(field, 0), msg[0], msg[1], msg[2]) } }
    let(:messages) { dn_messages + [rate_message] + report_messages }
    ...
    context 'Diese 3', vcr: { cassette_name: 'diese_3_171579' } do
      let(:dossier_nb) { 171_579 }
